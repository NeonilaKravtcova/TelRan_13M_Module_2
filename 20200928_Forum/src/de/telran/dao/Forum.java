...
public class Forum {
    ...
    public Post getPostWithMaxLikes() {
        System.out.println("------------------------------------");
        System.out.println("Post with maximal quantity of likes:");
        System.out.println("------------------------------------");
        Post post = this.posts[0];
        for(int i = 0; i < this.size - 1; ++i) {
            if (post.getLikes() < this.posts[i + 1].getLikes()) {
                post = this.posts[i + 1];
            }
        }
        return post;
    ...
    //To class Forum add static method that accepts an array (any type) and prints out all not null elements.

    public static void displayNotNullElementsOfArray(Object[] array){
        System.out.println("--------------------------");
        System.out.println("List of requested elements");
        System.out.println("--------------------------");
        for (Object o : array){
            if (o != null){
